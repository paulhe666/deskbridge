...
path remains a beta feature. Its pointer capture and modifier state machines
were rewritten for v1.1.0, but the available real-device and multi-display test
matrix is still smaller than the Windows server path.
...
1. Download `Deskbridge-Setup-1.1.0.exe`.
...
1. Download `Deskbridge-1.1.0.pkg`.
...
Windows 边缘投递条。macOS 作为服务端目前仍属于 Beta 功能。v1.1.0 已经
重写指针捕获和修饰键状态机，但真实设备、多显示器和不同鼠标型号的测试矩阵
仍小于 Windows 服务端路径，后续还需要继续实机验证。
...
1. 下载 `Deskbridge-Setup-1.1.0.exe`。
2. 双击安装包并按安装向导安装。
